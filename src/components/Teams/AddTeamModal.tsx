...
const [subcategorias, setSubcategorias] = useState<Subcategoria[]>([]);
  ...
  useEffect(() => {
    const fetchModalidades = async () => {
      ...
      // Encontrar o id do esporte selecionado
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Buscar o id do esporte selecionado
      const esporte = sports.find(s => s.nome === formData.organization_type);
      const esporte_id = esporte ? esporte.id : null;
    ...
    >
      ...
      >
        <div className="flex justify-between items-center p-4 border-b sticky top-0 bg-white z-10 rounded-t-lg">
          <h2 className="text-xl font-semibold text-gray-800">Adicionar Nova Equipe</h2>
          <button 
            onClick={onClose}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  >
                    ...
                    {sports.map((sport) => (
                      <option key={sport.id} value={sport.nome}>{sport.nome}</option>
                    ))}
                  </select>
                </div>
